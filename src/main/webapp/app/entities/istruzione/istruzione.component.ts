...
export class IstruzioneComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.istruzioneService
      ...
      .subscribe(
        (res: HttpResponse<IIstruzione[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInIstruziones(): void {
    this.eventSubscriber = this.eventManager.subscribe('istruzioneListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IIstruzione[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/istruzione'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
