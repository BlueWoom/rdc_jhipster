...
export class EsperienzaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.esperienzaService
      ...
      .subscribe(
        (res: HttpResponse<IEsperienza[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInEsperienzas(): void {
    this.eventSubscriber = this.eventManager.subscribe('esperienzaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IEsperienza[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/esperienza'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
