...
export class OffertaOccupazioneRichiestaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.offertaOccupazioneRichiestaService
      ...
      .subscribe(
        (res: HttpResponse<IOffertaOccupazioneRichiesta[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInOffertaOccupazioneRichiestas(): void {
    this.eventSubscriber = this.eventManager.subscribe('offertaOccupazioneRichiestaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IOffertaOccupazioneRichiesta[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/offerta-occupazione-richiesta'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.offertaOccupazioneRichiestas = data || [];
